...
#' Function to calculate the  likelihood for multispecies occupancy models
#'
...
#'
#' @param detection_data three dimensional array of detection data, number of
#' sites by number of hosts by number of visits (J x N x K)
#' @param psi_samples three dimensional array of MCMC samples for occupancy
#' probabilities, with dimensions of number of sites by number of species by
#' number of iterations (J x N x S)
#' @param p_samples four dimensional array of MCMC samples for detection
...
#'
multispecies_likelihood = function(detection_data, psi_samples, p_samples) {

    # number of sites
    J <- dim(detection_data)[1]
    # number of species
    N <- dim(detection_data)[2]
    # number of iterations
    S <- dim(p_samples)[4]

    # J x N matrix of naive occupancy states for each species
    naive_state <- multispecies_naive_state(detection_data)
    ...
    for (s in 1:S) {
        for (j in 1:J) {
            for (i in 1:N) {
                ...
                ys = detection_data[j, i, ]
                ps = p_samples[j, i, , s]
                psi = psi_samples[j, i, s]

                # indicates whether species was detected at site
                detected_at_site <- naive_state[j, i]

                site_likelihood[j, i, s] <- calc_site_likelihood(
                    pr_detection = ps,
                    detection_history = ys,
                    pr_occupancy = psi,
                    detected_at_site = detected_at_site
                )
    ...
    # reformat array as data.frame
    site_likelihood <- reshape2::melt(
        site_likelihood,
        varnames = c('site', 'species', 'iteration'),
        value.name = 'likelihood'
    )
